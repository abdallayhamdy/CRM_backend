...
/* ─────────────────────────────────
   NOTE: This page is for GLOBAL contact settings.
   The [id] param is used ONLY for context (back button, breadcrumb).
   Settings are stored globally in localStorage (key: crm_panel_cards_contacts_v2)
   and apply to ALL contacts, not just the one referenced by [id].
───────────────────────────────── */
...
/* ─────────────────────────────────
   Draggable card row (right panel)
───────────────────────────────── */
function CardRow({
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1">
          ...
          {hasConditionalLogic && (
            <span className="text-[10px] bg-status-info-light text-primary px-1.5 py-0.5 rounded font-bold" title="Has conditional logic">
              C
            </span>
          )}
      ...
      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          ...
          className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors"
        >
            ...
            <div className="py-1 flex flex-col">
              {setConditionalLogicInitial && setConditionalLogicCardId && (
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    setConditionalLogicInitial(card.conditionalLogic || null)
                    setConditionalLogicCardId(card.id)
                  }}
                  className="w-full text-left px-4 py-2 text-[13px] text-foreground hover:bg-accent transition-colors flex items-center justify-between"
                >
                  Set conditional logic <span className="text-[10px]">›</span>
                </button>
              )}
              <button
                onClick={(e) => { e.stopPropagation(); onEdit(); document.dispatchEvent(new MouseEvent('mousedown')); }}
                className="w-full text-left px-4 py-2 text-[13px] text-foreground hover:bg-accent transition-colors"
              >
                Edit card
              </button>
              {onToggle && (
                <button
                  onClick={(e) => { e.stopPropagation(); onToggle(); document.dispatchEvent(new MouseEvent('mousedown')); }}
                  className={cn(
          ...
          <Popover>
            ...
            <PopoverContent className="z-[10000] w-[160px] p-0 rounded-md border border-border shadow-md animate-in fade-in zoom-in-95" align="end" sideOffset={8}>
              <div className="py-1 flex flex-col">
                ...
                {onRemove && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); onRemove(); document.dispatchEvent(new MouseEvent('mousedown')); }}
                    className="w-full text-left px-4 py-2 text-[13px] text-destructive hover:bg-destructive/10 transition-colors"
                  >
                    Remove card
                  </button>
                )}
...
/* ─────────────────────────────────
   Add Card Divider (hover between cards)
────────────────────────────────── */
...
───────────────────────────────── */
const LIBRARY_CARDS = [
  { id: "about",   label: "About this contact",        added: true  },
  { id: "keyinfo", label: "Key information",            added: true  },
]

function LibraryTab({ searchQuery, addedIds, onToggle }: {
  searchQuery: string
  addedIds: string[]
  ...
  const filtered = LIBRARY_CARDS.filter(c =>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <Popover>
          ...
          <PopoverContent className="z-[10000] w-[240px] p-0 rounded-md border border-border shadow-md animate-in fade-in zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95" align="start" sideOffset={8}>
            <div className="p-3 bg-muted/50 border-b border-border rounded-t-md">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search"
                  className="w-full pl-3 pr-9 py-1.5 text-[14px] border border-primary rounded-full focus:outline-none focus:ring-1 focus:ring-primary text-foreground"
                />
              </div>
            </div>
            <div className="p-2 space-y-0.5">
              <label className="flex items-center gap-3 px-2 py-1.5 hover:bg-accent rounded cursor-pointer group">
                <div className="w-4 h-4 border border-border rounded-xs bg-background group-hover:border-muted-foreground" />
                <span className="text-[14px] font-bold text-foreground">Select all</span>
              </label>
              <label className="flex items-center gap-3 px-2 py-1.5 hover:bg-accent rounded cursor-pointer group bg-muted/50">
                <div className="w-4 h-4 border border-border rounded-xs bg-background group-hover:border-muted-foreground" />
                <span className="text-[14px] text-foreground">Property list</span>
              </label>
              <label className="flex items-center gap-3 px-2 py-1.5 hover:bg-accent rounded cursor-pointer group">
                <div className="w-4 h-4 border border-border rounded-xs bg-background group-hover:border-muted-foreground" />
                <span className="text-[14px] text-foreground">Standard</span>
              </label>
            </div>
          </PopoverContent>
        </Popover>
        ...
        <div className="flex items-center gap-1 text-[13px] text-muted-foreground">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="font-bold text-foreground flex items-center gap-0.5 hover:underline">
                {sortBy}
                <svg className="w-3 h-3 fill-current" viewBox="0 0 12 12" aria-hidden="true"><path d="M6 8L1 3h10z"/></svg>
              </button>
            </PopoverTrigger>
            <PopoverContent className="z-[10000] w-[180px] p-0 rounded-md border border-border shadow-md animate-in fade-in zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95" align="end" sideOffset={8}>
              <div className="py-1">
                {["Newest", "Card name (A to Z)", "Card name (Z to A)"].map((opt, i) => (
                  <button
                    key={opt}
                    onClick={() => setSortBy(opt)}
                    className={cn(
                      "w-full text-left px-4 py-2 text-[14px] transition-colors hover:bg-accent",
                      opt === "Card name (Z to A)" ? "bg-muted/50 text-foreground" : "text-primary"
                    )}
                  >
                    {opt}
                  </button>
          ...
          return (
            <div key={card.id} className="border border-border rounded-lg p-3 flex flex-col gap-3 hover:shadow-sm transition-shadow bg-background">
              ...
              <div className="flex items-start justify-between gap-1">
                ...
                {/* Rootline CRM-style sprocket badge */}
                <svg className="w-4 h-4 shrink-0 text-primary" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 text-[11px] font-bold border rounded px-2 py-1 transition-colors flex-1 justify-center",
                    !isAdded && "border-border text-foreground hover:bg-accent",
                    isAdded && hoveredCardId !== card.id && "border-primary text-primary",
                    isAdded && hoveredCardId === card.id && "border-destructive/30 text-destructive bg-destructive/10 hover:bg-destructive/15"
                  )}
                >
                  {isAdded ? (
                    hoveredCardId === card.id ? (
                      ...
                      <><svg className="w-3 h-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M2 6l3 3 5-5"/></svg> Added</>
                    )
                  ) : (
                    <><span className="text-[13px] leading-none">+</span> Add card</>
                  )}
                </button>
                <button className="text-[11px] font-bold border border-border rounded px-2 py-1 text-foreground hover:bg-accent transition-colors">
                  Preview
                </button>
              </div>
            </div>
...
const DRAWER_TITLES: Record<DrawerTarget, string> = {
  left:   "Add left sidebar cards",
  right:  "Add right sidebar cards",
  center: "Add center panel cards",
}

const UNEDITABLE_CARD_PREFIXES = [
  'social', 'credit_memos', 'attribution', 'sales_nav', 'segments',
  'marketing', 'invoices', 'playbooks', 'wf_mems'
]

const isUneditableCard = (cardId: string) => {
  return UNEDITABLE_CARD_PREFIXES.some(prefix => cardId.startsWith(prefix))
}
    ...
    <Sheet open={open} onOpenChange={(isOpen) => { if (!isOpen) { setDrawerTab("create"); setSearchQuery(""); onClose() } }}>
      <SheetContent side="right" className="p-0 max-w-[90vw] data-[side=right]:w-[600px] data-[side=right]:sm:max-w-[600px]" showCloseButton={false}>
        ...
        <div className="flex border-b border-border shrink-0">
          {(["create", "library"] as const).map(t => (
            <button
              ...
              className={cn(
                "flex-1 py-2.5 text-[13px] font-bold transition-all relative",
                drawerTab === t
        ...
        <div className="px-4 py-3 border-b border-border shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-[13px] border border-border rounded-full focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary placeholder:text-muted-foreground text-foreground"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
                ...
                <div className="p-4 flex flex-col gap-4">
                  <div>
                    ...
                    <p className="text-[13px] text-foreground leading-relaxed">
                      Show a list of up to 50 <span onClick={onEditProperties} className="cursor-pointer hover:underline">editable properties</span>.
                    </p>
                  </div>
                  <button onClick={onEditProperties} className="w-full py-1 text-[13px] text-foreground bg-background border border-border rounded-xs hover:bg-accent transition-colors">
                    Create card
                  </button>
                </div>
              </div>
          ...
          {drawerTab === "library" && (
            <LibraryTab searchQuery={searchQuery} addedIds={addedIds} onToggle={onToggle} />
          )}
        </div>
      </SheetContent>
    </Sheet>
...
export default function LayoutEditorPage() {
  ...
  // NOTE: id is used ONLY for context (back button, breadcrumb)
  // Settings are GLOBAL and stored in localStorage
  const id = params?.id as string

  const { cards, customLeftCards, customRightCards: persistedRightCards, leftAddedIds: persistedLeftIds, save, ready } = usePanelCards()
  ...
  const conditionalLogicCard = React.useMemo(() => {
    ...
    return draft.find(c => c.id === conditionalLogicCardId) ||
           customRightCards.find(c => c.id === conditionalLogicCardId)
  ...
  const conditionalLogicProperties = React.useMemo(() => {
    ...
    return [{
      ...
      items: conditionalLogicCard.properties.map(p => ({ id: String(p.id ?? ''), label: String(p.label ?? '') }))
    }]
  ...
  React.useEffect(() => {
    if (ready && editParam) {
      // Only open EditCardDrawer for custom cards (not default cards handled by EditDefaultCardView)
      const isDefaultCard = draft.some(c => c.id === editParam)
      if (!isDefaultCard) {
        setTimeout(() => setEditCardId(editParam), 100)
      }
    }
  }, [ready, editParam, draft])
  const toggleDraft = (cardId: string) => {
    setDraft(p => p.map(c => c.id === cardId ? { ...c, enabled: !c.enabled } : c))
    setSaved(false)
  }
  ...
  const handleSave = async (exit = false) => {
    // Preserve current state for potential revert
    const prevDraft = [...draft]
    const prevCustomCards = [...customCards]
    const prevLeftAddedIds = [...leftAddedIds]
    const prevCustomRightCards = [...customRightCards]
    
    try {
      await save(draft, customCards, leftAddedIds, customRightCards)
      setSaved(true)
      if (exit) {
        setTimeout(() => router.push(`/contacts/${id}`), 400)
      } else {
      ...
      setDraft(prevDraft)
      setCustomCards(prevCustomCards)
      setLeftAddedIds(prevLeftAddedIds)
      setCustomRightCards(prevCustomRightCards)
      setSaved(false)
  ...
  const handleSaveConditionalLogic = (condition: any) => {
    ...
    setDraft(prev => prev.map(c =>
      c.id === conditionalLogicCardId
        ? { ...c, conditionalLogic: condition as ConditionalLogic }
        : c
    ));

    setCustomRightCards(prev => prev.map(c =>
      c.id === conditionalLogicCardId
        ? { ...c, conditionalLogic: condition as ConditionalLogic }
        : c
    ));

    setConditionalLogicCardId(null);
    setConditionalLogicInitial(null);
    setSaved(false);
  }
  ...
  return (
    <DragContext.Provider value={{ draggingId, dragOverId }}>
      ...
      <EditCardDrawer
        open={!!editCardId}
        onClose={() => { setEditCardId(null); router.push(`/contacts/${id}/settings`) }}
        onSave={(title, properties) => {
          if (editCardId === "new") {
            const newId = `custom_${Date.now()}`;
            const updatedCards = [...customCards, { id: newId, label: title, properties }];
            setCustomCards(updatedCards);
            const updatedLeftIds = [...leftAddedIds, newId];
            setLeftAddedIds(updatedLeftIds);
            save(draft, updatedCards, updatedLeftIds, customRightCards);
            setSaved(true);
            setTimeout(() => setSaved(false), 2000);
          } else if (editCardId && customRightCards.some(c => c.id === editCardId)) {
            const updated = customRightCards.map(c => c.id === editCardId ? { ...c, label: title, properties: properties?.map(p => ({ id: p.id, label: p.label })) } : c);
            setCustomRightCards(updated);
            save(draft, customCards, leftAddedIds, updated);
            setSaved(true);
            setTimeout(() => setSaved(false), 2000);
          } else if (editCardId && editCardId.startsWith("custom_")) {
            const updated = customCards.map(c => c.id === editCardId ? { ...c, label: title, properties } : c);
            setCustomCards(updated);
            save(draft, updated, leftAddedIds, customRightCards);
            setSaved(true);
            setTimeout(() => setSaved(false), 2000);
          }
        }}
        ...
        initialTitle={
          editCardId === "new"
            ? "Property list"
            : customRightCards.find(c => c.id === editCardId)?.label
              || customCards.find(c => c.id === editCardId)?.label
        }
        initialProperties={
          editCardId === "new"
            ? undefined
            : customRightCards.find(c => c.id === editCardId)?.properties
              || customCards.find(c => c.id === editCardId)?.properties
        }
      />
      <div className="h-screen bg-muted/50 flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <header className="bg-background border-b border-border h-[52px] flex items-center px-5 relative shrink-0 z-10 shadow-sm">
          <button
            ...
            className="px-4 py-1.5 text-[13px] font-bold text-foreground border border-border rounded hover:bg-accent transition-colors absolute left-5"
          >
            Exit
          </button>

          <div className="mx-auto flex items-center gap-2">
            <span className="text-[14px] font-bold text-foreground">Contact record page</span>
          ...
          <div className="flex flex-1 overflow-hidden gap-px bg-border">
            ...
            <div className="w-[260px] shrink-0 bg-muted/50 flex flex-col">
              <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 flex flex-col">
                ...
                <FixedLeftCard label="About this contact" desc="About this contact updated at …" onEdit={() => {
                  const card = draft.find(c => c.id === "about")
                  if (card) {
                    setEditCardId(null)
                    setEditingDefaultCard(card)
                    router.push(`/contacts/${id}/settings?edit=about`)
                  }
                }} />

                <AddCardDivider onAdd={() => openDrawer("left")} />
                <FixedLeftCard label="Key information" desc="Key information · 0-1 · template" onEdit={() => {
                  const card = draft.find(c => c.id === "keyinfo")
                  if (card) {
                    setEditCardId(null)
                    setEditingDefaultCard(card)
                    router.push(`/contacts/${id}/settings?edit=keyinfo`)
                  }
                }} />

                {leftAddedIds
                  .filter(leftId => !["about", "keyinfo"].includes(leftId))
                  .map(leftId => {
                    ...
                    return (
                      <React.Fragment key={leftId}>
                        ...
                        <div onClick={(e) => {
                          ...
                          router.push(`/contacts/${id}/settings?edit=${leftId}`)
                        }} className="flex items-center gap-2 px-3 py-2.5 bg-background border border-border rounded-md group select-none animate-in fade-in slide-in-from-top-1 duration-200 cursor-pointer">
                          <div className="shrink-0 text-border">
                            <svg width="10" height="16" viewBox="0 0 10 16" fill="currentColor" aria-hidden="true">
                              <circle cx="2" cy="3" r="1.5"/><circle cx="8" cy="3" r="1.5"/>
                              <circle cx="2" cy="8" r="1.5"/><circle cx="8" cy="8" r="1.5"/>
                          ...
                          <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleLibraryToggle(leftId)} className="w-8 h-8 flex items-center justify-center text-destructive/70 hover:text-destructive hover:bg-destructive/10 rounded transition-colors" title="Remove card">
                              <X className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => setEditCardId(leftId)} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent rounded">
                              <Pencil className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </div>
            ...
            <div className="flex-1 bg-muted/50 overflow-y-auto p-4 flex flex-col gap-3">
              ...
              <div className="bg-background border border-border rounded-md flex items-center justify-between px-4 py-2.5 shadow-sm">
                ...
                <div className="flex items-center gap-2 text-muted-foreground/60">
                  <button aria-label="Add activity" className="w-8 h-8 flex items-center justify-center hover:text-foreground transition-colors rounded hover:bg-accent">
                  </button>
                  <button aria-label="Refresh activities" className="w-8 h-8 flex items-center justify-center hover:text-foreground transition-colors rounded hover:bg-accent">
              ...
              <div className="bg-background border border-border rounded-md p-6 shadow-sm flex flex-col items-center text-center flex-1">
                ...
                <div className="mt-2 opacity-80">
                  <svg width="200" height="140" viewBox="0 0 200 140" fill="none" aria-hidden="true">
                    ...
                    {/* Flower pot */}
                    <ellipse cx="170" cy="126" rx="16" ry="6" fill="hsl(var(--border))" />
                    <rect x="157" y="110" width="26" height="18" rx="4" fill="hsl(var(--muted))" />
                    <circle cx="163" cy="104" r="8" fill="hsl(var(--status-success-light))" />
                    <circle cx="170" cy="100" r="10" fill="hsl(var(--status-success-light))" />
                    <circle cx="177" cy="104" r="7" fill="hsl(var(--status-success-light))" />
                    {/* Flowers */}
                    <circle cx="163" cy="96" r="4" fill="hsl(var(--status-danger-light))" />
                    <circle cx="170" cy="92" r="4" fill="hsl(var(--status-purple-light))" />
                    <circle cx="177" cy="96" r="3.5" fill="hsl(var(--status-danger-light))" />
                  </svg>
                </div>
              </div>
            </div>
                ...
                {draft.map(card => (
                  <React.Fragment key={card.id}>
                    <CardRow
                      ...
                      onEdit={() => { setEditCardId(null); setEditingDefaultCard(card); router.push(`/contacts/${id}/settings?edit=${card.id}`) }}
                      onToggle={() => toggleDraft(card.id)}
                      setConditionalLogicInitial={setConditionalLogicInitial}
                      setConditionalLogicCardId={setConditionalLogicCardId}
                    />
                    <AddCardDivider onAdd={() => setRightDrawerOpen(true)} />
                  </React.Fragment>
                ))}
                {customRightCards.map(card => {
                  ...
                  <React.Fragment key={card.id}>
                    <div className="flex items-center gap-2 px-3 py-2.5 bg-background border border-border rounded-md group select-none animate-in fade-in slide-in-from-right-1 duration-200 cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1"><span className="text-[13px] font-bold text-foreground">{card.label}</span>
                        {hasConditionalLogic && (
                          <span className="text-[10px] bg-status-info-light text-primary px-1.5 py-0.5 rounded font-bold" title="Has conditional logic">
                            C
                          </span>
                      ...
                      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!isUneditableCard(card.id) && (
                          <button onClick={() => router.push(`/contacts/${id}/settings?edit=${card.id}`)} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors" title="Edit card"><Pencil className="w-3.5 h-3.5" /></button>
                        )}
        ...
        <AddRightCardDrawer
          ...
          existingCardIds={[...draft.filter(c => c.enabled !== false).map(c => c.id), ...customRightCards.map(c => c.id)]}
          onSave={(newCard) => {
            const disabledDraft = draft.find(c => c.enabled === false && newCard.id.startsWith(c.id + '_'))
            if (disabledDraft) {
              const updatedDraft = draft.map(c => c.id === disabledDraft.id ? { ...c, enabled: true } : c)
              setDraft(updatedDraft)
              save(updatedDraft, customCards, leftAddedIds, customRightCards)
            } else {
              const updated = [...customRightCards, newCard]
              setCustomRightCards(updated)
              save(draft, customCards, leftAddedIds, updated)
            }
            setSaved(true)
          ...
          onRemove={(cardId) => {
            if (draft.some(c => c.id === cardId)) {
              const updated = draft.map(c => c.id === cardId ? { ...c, enabled: false } : c)
              setDraft(updated)
              save(updated, customCards, leftAddedIds, customRightCards)
            } else {
              const updated = customRightCards.filter(c => c.id !== cardId)
              setCustomRightCards(updated)
              save(draft, customCards, leftAddedIds, updated)
            }
            setSaved(true)
        ...
        {createCardViewOpen && (
          <CreateRightCardView
            type={createCardType}
            onClose={() => setCreateCardViewOpen(false)}
            onSave={(newCard) => {
              const updated = [...customRightCards, newCard]
              setCustomRightCards(updated)
              save(draft, customCards, leftAddedIds, updated)
              setCreateCardViewOpen(false)
              setSaved(true)
              setTimeout(() => setSaved(false), 2000)
            }}
          />
        ...
        {editingDefaultCard && (
          <EditDefaultCardView
            card={editingDefaultCard}
            onClose={() => { setEditingDefaultCard(null); router.push(`/contacts/${id}/settings`) }}
            onSave={(updatedCard) => {
              setDraft(prev => prev.map(c => c.id === updatedCard.id ? { ...c, label: updatedCard.label, properties: updatedCard.properties?.map(p => ({ id: p.id, label: p.name })) } : c))
        ...
        {conditionalLogicCardId && (
          <ConditionalLogicModal
            open={!!conditionalLogicCardId}
            onClose={() => {
              setConditionalLogicCardId(null)
              setConditionalLogicInitial(null)
            }}
            onSave={handleSaveConditionalLogic}
            initialCondition={conditionalLogicInitial || undefined}
            properties={conditionalLogicProperties}
          />
        )}
      </div>
