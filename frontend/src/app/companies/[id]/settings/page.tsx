...
/* ─────────────────────────────────
   NOTE: This page is for GLOBAL company settings.
   The [id] param is used ONLY for context (back button, breadcrumb).
   Settings are stored globally in localStorage (key: crm_panel_cards_companies)
   and apply to ALL companies, not just the one referenced by [id].
───────────────────────────────── */
...
───────────────────────────────── */
const LIBRARY_CARDS = [
  { id: "about",   label: "About this company",        added: true  },
...
export default function LayoutEditorPage() {
  ...
  const { cards, customLeftCards, customRightCards: persistedRightCards, leftAddedIds: persistedLeftIds, save, ready } = usePanelCards('companies')
  ...
  const handleSave = async (exit = false) => {
    ...
    try {
      ...
      setSaved(true)
      if (exit) {
        setTimeout(() => router.push(`/companies/${id}`), 400)
      } else {
  ...
  return (
    <DragContext.Provider value={{ draggingId, dragOverId }}>
      ...
      <EditCardDrawer
        open={!!editCardId}
        onClose={() => { setEditCardId(null); router.push(`/companies/${id}/settings`) }}
      ...
      <div className="h-screen bg-muted/50 flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <header className="bg-background border-b border-border h-[52px] flex items-center px-5 relative shrink-0 z-10 shadow-sm">
          ...
          <div className="mx-auto flex items-center gap-2">
            <span className="text-[14px] font-bold text-foreground">Company record page</span>
          ...
          <div className="flex flex-1 overflow-hidden gap-px bg-border">
            ...
            <div className="w-[260px] shrink-0 bg-muted/50 flex flex-col">
              <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 flex flex-col">
                ...
                <FixedLeftCard label="About this company" desc="About this company updated at …" onEdit={() => {
                  const card = draft.find(c => c.id === "about")
                  if (card) {
                    setEditCardId(null)
                    setEditingDefaultCard(card)
                    router.push(`/companies/${id}/settings?edit=about`)
                  }
                }} />
                ...
                <FixedLeftCard label="Key information" desc="Key information · 0-1 · template" onEdit={() => {
                  ...
                  if (card) {
                    setEditCardId(null)
                    setEditingDefaultCard(card)
                    router.push(`/companies/${id}/settings?edit=keyinfo`)
                  }
                }} />
                    ...
                    return (
                      <React.Fragment key={leftId}>
                        ...
                        <div onClick={(e) => {
                          ...
                          router.push(`/companies/${id}/settings?edit=${leftId}`)
                ...
                {draft.map(card => (
                  <React.Fragment key={card.id}>
                    <CardRow
                      ...
                      onEdit={() => { setEditCardId(null); setEditingDefaultCard(card); router.push(`/companies/${id}/settings?edit=${card.id}`) }}
                    ...
                    <div className="flex items-center gap-2 px-3 py-2.5 bg-background border border-border rounded-md group select-none animate-in fade-in slide-in-from-right-1 duration-200 cursor-pointer">
                      ...
                      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!isUneditableCard(card.id) && (
                          <button onClick={() => router.push(`/companies/${id}/settings?edit=${card.id}`)} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors" title="Edit card"><Pencil className="w-3.5 h-3.5" /></button>
                        )}
        ...
        {editingDefaultCard && (
          <EditDefaultCardView
            ...
            onClose={() => { setEditingDefaultCard(null); router.push(`/companies/${id}/settings`) }}
            onSave={(updatedCard) => {
