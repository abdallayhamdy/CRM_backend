...
import { X, Search, CheckSquare, Square, ChevronDown, ChevronUp, Building2, Loader2 } from "lucide-react"
...
interface AddCompanyToDealSheetProps {
  open: boolean
  onClose: () => void
  dealId: string
  dealTitle?: string
...
export function AddCompanyToDealSheet({
  open,
  onClose,
  dealId,
  dealTitle,
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      setCompanies((data as unknown as CompanyResult[]) || [])
      setLoading(false)
    }
    load()
...
function InlineCreateCompanyForm({
  ...
  const handleCreate = async () => {
    ...
    setSaving(false)
    if (updateError) {
      toast.error("Company created but failed to associate with deal")
      return
    }
