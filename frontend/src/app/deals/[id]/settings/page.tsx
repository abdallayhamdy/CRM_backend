...
const LIBRARY_CARDS = [
  { id: "about",   label: "About this deal",        added: true  },
...
export default function LayoutEditorPage() {
  ...
  const { cards, customLeftCards, customRightCards: persistedRightCards, leftAddedIds: persistedLeftIds, save, ready } = usePanelCards('deals')
  ...
  const handleSave = async (exit = false) => {
    ...
    try {
      ...
      if (exit) setTimeout(() => router.push(`/deals/${id}`), 400); else setTimeout(() => setSaved(false), 2000)
  ...
  return (
    <DragContext.Provider value={{ draggingId, dragOverId }}>
      ...
      <EditCardDrawer
        open={!!editCardId}
        onClose={() => { setEditCardId(null); router.push(`/deals/${id}/settings`) }}
      ...
      <div className="h-screen bg-muted/50 flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
        <header className="bg-background border-b border-border h-[52px] flex items-center px-5 relative shrink-0 z-10 shadow-sm">
          <button onClick={() => router.push(`/deals/${id}`)} className="px-4 py-1.5 text-[13px] font-bold text-foreground border border-border rounded hover:bg-accent transition-colors absolute left-5">Exit</button>
          <div className="mx-auto flex items-center gap-2">
            <span className="text-[14px] font-bold text-foreground">Deal record page</span>
          ...
          <div className="flex flex-1 overflow-hidden gap-px bg-border">
            <div className="w-[260px] shrink-0 bg-muted/50 flex flex-col">
              <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 flex flex-col">
                ...
                <FixedLeftCard label="About this deal" desc="About this deal updated at …" onEdit={() => { const card = draft.find(c => c.id === "about"); if (card) { setEditCardId(null); setEditingDefaultCard(card); router.push(`/deals/${id}/settings?edit=about`) } }} />
                <AddCardDivider onAdd={() => openDrawer("left")} />
                <FixedLeftCard label="Key information" desc="Key information · 0-1 · template" onEdit={() => { const card = draft.find(c => c.id === "keyinfo"); if (card) { setEditCardId(null); setEditingDefaultCard(card); router.push(`/deals/${id}/settings?edit=keyinfo`) } }} />
                {leftAddedIds.filter(leftId => !["about", "keyinfo"].includes(leftId)).map(leftId => {
                  ...
                  return (
                    <React.Fragment key={leftId}>
                      <AddCardDivider onAdd={() => openDrawer("left")} />
                      <div onClick={(e) => { if ((e.target as HTMLElement).closest('button')) return; router.push(`/deals/${id}/settings?edit=${leftId}`) }} className="flex items-center gap-2 px-3 py-2.5 bg-background border border-border rounded-md group select-none animate-in fade-in slide-in-from-top-1 duration-200 cursor-pointer">
                  ...
                  return (
                    <React.Fragment key={card.id}>
                      <div className="flex items-center gap-2 px-3 py-2.5 bg-background border border-border rounded-md group select-none animate-in fade-in slide-in-from-right-1 duration-200 cursor-pointer">
                        ...
                        <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                          {!isUneditableCard(card.id) && <button onClick={() => router.push(`/deals/${id}/settings?edit=${card.id}`)} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors" title="Edit card"><Pencil className="w-3.5 h-3.5" /></button>}
