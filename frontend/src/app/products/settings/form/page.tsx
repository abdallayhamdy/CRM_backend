...
const INITIAL_GROUPS: PropertyGroup[] = [
  {
    ...
    properties: [
      { id: "name", label: "Product name", required: true, selected: true, type: "text" },
      { id: "sku", label: "SKU", selected: true, type: "text" },
      { id: "unit_price", label: "Unit price", required: true, selected: true, type: "number" },
      { id: "status", label: "Status", selected: true, type: "select" },
      { id: "product_type", label: "Product type", selected: false, type: "select" },
      { id: "product_description", label: "Description", selected: false, type: "textarea" },
      { id: "unit_cost", label: "Unit cost", selected: false, type: "number" },
      { id: "billing_frequency", label: "Billing frequency", selected: false, type: "select" },
      { id: "url", label: "Product URL", selected: false, type: "url" },
      { id: "product_image_url", label: "Image URL", selected: false, type: "url" },
      { id: "product_folder", label: "Product folder", selected: false, type: "text" },
    ]
  }
]
...
function FieldItem({
  ...
  return (
    <div className="group relative transition-all duration-200">
      <div className="space-y-2 transition-all duration-200">
        ...
        <div className="space-y-1.5">
          {prop.type === "textarea" ? (
            ...
            <Input type="url" className="h-[38px] w-full rounded-xs border border-border bg-background px-2 transition-all focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 group-hover:border-border" />
          ) : (
      ...
      <div className="absolute right-0 -top-5 opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-center bg-background border border-border rounded-xs shadow-[0_4px_12px_rgba(0,0,0,0.1)] overflow-hidden z-20">
        <Button
          variant="ghost"
          size="icon"
...
export default function EditProductFormPage() {
  ...
  const { formFields, setFormFields, loading, saving, hasChanges, save, reset: resetLayout, updateFormField } = useFormLayout('product', defaultSelected)
  const [groups, setGroups] = React.useState<PropertyGroup[]>(INITIAL_GROUPS)
  const [searchQuery, setSearchQuery] = React.useState("")
  const [expandedGroups, setExpandedGroups] = React.useState<string[]>(["product-info"])
  ...
  const handleSave = async (shouldClose = false) => {
    ...
    if (success) {
      toast.success("Product form layout saved")
      if (shouldClose) router.push("/products")
    } else {
    ...
    <div className="flex flex-col h-[125vh] w-full max-w-screen-xl bg-muted/50 font-sans" style={{ zoom: 0.8 }}>
      ...
      <header className="h-[52px] bg-foreground flex items-center justify-between px-4 shrink-0 text-primary-foreground shadow-sm z-50">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[14px] font-bold tracking-tight">Edit Product form</h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            variant="ghost"
            className="text-primary-foreground hover:bg-[var(--color-hs-card-bg)]/10 h-9 px-4 text-[14px] font-bold transition-colors disabled:opacity-50"
            onClick={() => handleSave()}
          ...
          <Button
            className="bg-primary hover:bg-primary/90 text-primary-foreground h-9 px-6 text-[14px] font-bold border-none transition-all shadow-sm active:scale-95 disabled:opacity-50"
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-full sm:w-[340px] bg-background border-r border-border flex flex-col shrink-0 shadow-[2px_0_4px_rgba(0,0,0,0.02)] z-10">
          <div className="p-8 pb-5">
            ...
            <p className="text-[14px] text-muted-foreground mb-6 leading-relaxed">
              Select the fields your team will see when adding a new product record. <a href="#" className="text-primary font-bold hover:underline inline-flex items-center gap-1">Docs <ExternalLink className="h-3 w-3" /></a>
            </p>
        ...
        <div className="flex-1 flex flex-col bg-muted/50 overflow-y-auto crm-scrollbar relative">
          <div className="w-full max-w-[640px] mx-auto py-12 px-6">
            <div className="bg-background border border-border rounded-sm shadow-[0_2px_8px_rgba(0,0,0,0.05)] flex flex-col min-h-[850px] mb-20">
              <div className="px-10 py-7 border-b border-border">
                <h2 className="text-[20px] font-bold text-foreground tracking-tight">Create product</h2>
              </div>
        ...
        <div className="w-[56px] bg-background border-l border-border flex flex-col items-center py-8 gap-6 shadow-[-2px_0_4px_rgba(0,0,0,0.02)] z-10">
