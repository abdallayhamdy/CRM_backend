...
const INITIAL_GROUPS: PropertyGroup[] = [
  {
    ...
    properties: [
      { id: "order_number", label: "Order number", required: true, selected: true, type: "text" },
      { id: "status", label: "Status", required: true, selected: true, type: "select" },
      { id: "amount", label: "Amount", selected: true, type: "number" },
      { id: "currency", label: "Currency", selected: true, type: "select" },
      { id: "closed_at", label: "Close date", selected: false, type: "date" },
      { id: "order_description", label: "Description", selected: false, type: "textarea" },
    ]
  },
  {
    id: "order-associations",
    label: "Associations",
    properties: [
      { id: "contact_id", label: "Contact", selected: true, type: "select" },
      { id: "company_id", label: "Company", selected: false, type: "select" },
      { id: "deal_id", label: "Deal", selected: false, type: "select" },
    ]
  }
]
...
export default function EditOrderFormPage() {
  ...
  const handleSave = async (shouldClose = false) => {
    ...
    if (success) {
      toast.success("Order form layout saved")
      if (shouldClose) router.push("/orders")
    } else {
    ...
    <div className="flex flex-col h-[125vh] w-full max-w-screen-xl bg-muted/50 font-sans" style={{ zoom: 0.8 }}>
      ...
      <header className="h-[52px] bg-foreground flex items-center justify-between px-4 shrink-0 text-primary-foreground shadow-sm z-50">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[14px] font-bold tracking-tight">Edit Order form</h1>
        </div>
        ...
        <div className="w-full sm:w-[340px] bg-background border-r border-border flex flex-col shrink-0 shadow-[2px_0_4px_rgba(0,0,0,0.02)] z-10">
          <div className="p-8 pb-5">
            ...
            <p className="text-[14px] text-muted-foreground mb-6 leading-relaxed">
              Select the fields your team will see when adding a new order record. <a href="#" className="text-primary font-bold hover:underline inline-flex items-center gap-1">Docs <ExternalLink className="h-3 w-3" /></a>
            </p>
          ...
          <div className="w-full max-w-[640px] mx-auto py-12 px-6">
            <div className="bg-background border border-border rounded-sm shadow-[0_2px_8px_rgba(0,0,0,0.05)] flex flex-col min-h-[850px] mb-20">
              <div className="px-10 py-7 border-b border-border">
                <h2 className="text-[20px] font-bold text-foreground tracking-tight">Create order</h2>
              </div>
