...
import { MoreFilterCategory, MoreFilterType } from "@/lib/crm-properties"
...
interface CrmFilterChipRowProps {
  ...
  // Dynamic quick filters sourced from real DB properties. Never falls back to a
  // hardcoded list — if no real properties are available, the "+ More" button is hidden.
...
export function CrmFilterChipRow({
  ...
  const hasRealMoreFilters = !!moreFilters && moreFilters.length > 0

  const filteredMoreFilters = (moreFilters || [])
    .map(group => ({
      ...group,
      items: group.items.filter(item =>
        item.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !pinnedFilterIds.includes(item.id) &&
        item.id !== "lifecycle_stage"
      )
    }))
  ...
  return (
    <div className={cn("flex items-center gap-1 px-4 py-2 overflow-x-auto", className)}>
      <div className="flex items-center gap-1 overflow-x-auto no-scrollbar flex-1">
        ...
        {showMoreButton && hasRealMoreFilters && (
            ...
            {editFiltersView ? (
              ...
              <>
                ...
                <div className="flex flex-col py-2 max-h-[350px] overflow-y-auto">
                  ...
                  {filteredMoreFilters.length === 0 && (
                    <p className="px-5 py-4 text-[13px] text-muted-foreground">No filters match your search</p>
                  )}
                </div>
              </>
            )}
