...
import { ServiceResponse, DashboardOverview } from '@/lib/types/crm'
...
export const dashboardService = {
  async getOverview(workspaceId: string) {
    ...
    if (response.error) return { data: null, error: { message: response.error } } as ServiceResponse<DashboardOverview>
    return { data: response.data?.data ?? response.data, error: null } as ServiceResponse<DashboardOverview>
  },
